...
% Add origin node at the beginning
origin_node = [0, 0, 0];
nodes = [origin_node; central_node];  % Origin is node 1, central node is node 2
edges = [];

% Create edge from origin to central node
edges = [edges; 1, 2];
...
for branch = 1:n_branches
    ...
    start_idx = 2 + (branch-1)*(n_spaced-1);
    for i = 0:n_spaced-2
        if i == 0
...
% *** MODIFIED: Get the directory where this script is located ***
